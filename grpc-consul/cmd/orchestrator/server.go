...
type processServiceServerImpl struct {
...
func (s *processServiceServerImpl) Process(ctx context.Context, request *process.ProcessRequest) (*process.ProcessResponse, error) {
...
func (s *processServiceServerImpl) doProcessing1(request *process.ProcessRequest) (uint64, error) {
	...
	defer cancel2()
	if _, err = s.processorPrint.Process(ctx2, newVal); err != nil {
		return 0, err
	}
...
func (s *processServiceServerImpl) doProcessing2(request *process.ProcessRequest) (uint64, error) {
	...
	ctx, cancel := context.WithTimeout(context.TODO(), time.Second*4)
	defer cancel()
	if _, err := s.processorPrint.Process(ctx, newVal); err != nil {
		return 0, err
	}
